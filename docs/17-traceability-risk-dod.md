...
| R-14 | Platforma özel Docker sorunu | Orta | Orta | Portable paths/smoke |
...
- `docker compose up --build` verified clean (`docker compose down -v && up --build -d`): `db` healthy, `app` healthy/Up, `GET /actuator/health` -> `{"status":"UP"}`.
